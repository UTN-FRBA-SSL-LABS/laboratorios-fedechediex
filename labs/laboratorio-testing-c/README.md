...
> R:Inicializa un carrito en 0, si no la llamaramos antes de usar el carrito entonces cantidad puede tener un valor basura y puede contar mal la cant de productos, escribir en posiciones invalidas, leer memoria nunca inicializada.
...
> R:falls, esperaba 700 y obtuvo 350
...
> R:ademas de obtener el precio unitario faltaria multiplicarlo por la cantidad de elementos
...
> R:29 Y 30 ya que en test_unitarios.c no se llama a la funcion de carrito_descuento
...
> R:test unitario es para una funcion aislada, mientras que la integracion prueba varias funciones trabajando en conjunto. El unitario ya que probaba el calculo total con mas de 1 cantidad. 
...
> R: C no hay control de límites ni protección automática de memoria, mientras que en Python y Java el runtime valida y gestiona la memoria, por lo que no se puede accedes fuera de rango
...
> R:  Con los test podemos detectar errores de logica/calculo u otros de forma efectiva, mientras que mirando el codigo podes llegar a "simular" la ejecucion del codigo en tu cabeza pero es mas probable no detectar errores finos.
...
> R:  el codigo sumaba solo el precio unitario y como la cantidad era 1 daba correcto pero al poner 2 ese precio nunca se multiplicaba
